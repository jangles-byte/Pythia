...
| `GlobalHealthScore.tsx` | `src/components/GlobalHealthScore.tsx` — **PYTHIA's Global Health Score**: always-on top-left corner badge (big 1-100 number colored by band) that expands to the six-pillar breakdown (bars + drivers) + method. Fetches `/api/engine/health-score`; renders nothing until the engine serves a score. Rendered once (desktop) in `page.tsx` (`{!isMobile && <GlobalHealthScore/>}`) |
| `routes/planet-vitals-route.ts` | `src/app/api/planet-vitals/route.ts` — planetary vitals (no key; **a UA is required or NASA/NOAA 403**): atmospheric CO₂ (NOAA Mauna Loa global trend) + global temp anomaly (NASA GISTEMP). 6h cache |
