...
| `ContractsWindow.tsx` | `src/components/ContractsWindow.tsx` — the floating **federal-money tape** (tool strip, Coins icon): **AWARDED** tab (USASpending contract awards with amount/agency/recipient) + **OPEN** tab (Grants.gov open opportunities with close dates); every row links out. Rendered via the `'contracts'` FloatingWindow kind in `page.tsx` |
