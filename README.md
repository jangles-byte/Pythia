...
- 🛰 **Satellites at real altitude** — a 35,786 km GEO bird no longer sits on the same surface as a traffic camera. Satellites are drawn above the globe in a WebGL layer that uses MapLibre's own globe projection, so they keep the planet's curvature and survive the 2D switch; altitude is spread by a sqrt curve so the crowded LEO shell reads as a shell rather than a line. **Click one** for its readout and its orbit track.
- 📹 **Cameras, worldwide** — click **Cams near** on any event or forecast to pull the nearest of **~30,000 public cameras** into a live grid. It used to search North America only and come up empty in Bangkok, Athens or Taipei; it now reads Osiris's 45-source global directory (Europe, Japan, Taiwan, Hong Kong, Thailand, Spain, Australia, the US southern tier…) *and* PYTHIA's own supplement (NYC TMC, DelDOT, and 511 for NY/PA/WI/ID/AK/New England). Plus a **14-view NOAA GOES** satellite window.
...
- **Lifts the sky into 3D** — satellites are drawn **above the globe at their real altitude**, not pinned to the ground. A MapLibre custom WebGL layer positions each one through the globe projection's own `projectTileFor3D` prelude, so they inherit the planet's curvature and camera — and the same shader compiles for mercator, so the layer keeps working the moment you press 2D. True altitude is mapped onto a display band by a **sqrt** curve: low enough to clear the globe's depth test, high enough that GEO stays inside the frustum, and spread so the 161–2,000 km LEO shell doesn't collapse into a single line. The popup shows the real kilometres. **Click a satellite** for its readout and its orbit track. No extra 3D engine.
- **Pulls the cameras** — click **cams** on any event or **Cams near** on any forecast and PYTHIA finds the nearest public cameras (**~30,000** across 45 regional sources — Caltrans, TfL, Canada, the US southern tier, Spain, Germany, France, Italy, Japan, Taiwan, Hong Kong, Thailand, Australia, NZ… — plus PYTHIA's own NYC TMC, DelDOT and 511 for NY/PA/WI/ID/AK/New England) in an auto-refreshing still grid — the view nobody else has. Plus a **live NOAA GOES satellite** window: **14 views** — East & West CONUS/full-disk, regional sectors (Northeast, Southeast, Tropical Atlantic, US West, Hawaii), the roaming mesoscale sectors NOAA trains on active storms, and alternate bands (Infrared for night, AirMass, Sandwich). All keyless.
...
- **Movement & eyes** — flights (commercial / private / military) and satellites (**drawn at real altitude, clickable, with orbit tracks**), maritime traffic & chokepoints, **FAA airspace status** (ground stops/delays), surveillance balloons, live news streams, and **~30,000 public traffic/agency cameras worldwide** + **live NOAA GOES satellite imagery**.
