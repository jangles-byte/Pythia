"""Header-size variant of the halftone eye: tighter crop + chunkier dots so it
still reads as the eye at 36-44px. 512px RGBA, transparent rounded tile.
Writes to $OSIRIS_DIR/public/pythia-logo.png (default ~/osiris)."""
import math, os, struct, zlib

OUT = 512; SS = 2; S = OUT * SS
N = 32                       # chunky grid — dots survive small sizes
sp = S / N
maxr = sp * 0.48
...
cx = cy = N / 2
W = N * 0.47                 # eye fills the tile
H = W * 0.62                 # taller opening for the square crop
IR = H * 0.78; PUP = 0.30
...
def lum_at(gx, gy):
    dx, dy = gx - cx, gy - cy
    t = (dx + W) / (2 * W)
    if not (0 <= t <= 1): return 0.08
    lid_h = 4 * H * t * (1 - t)
    if abs(dy) > lid_h: return 0.08
    r = math.hypot(dx, dy)
    if r <= IR:
        if math.hypot(dx + IR*0.40, dy + IR*0.44) <= 0.15*IR: return 1.0
        ...
        for lat, lon, rad in LAND:
            ...
            if z2 <= 0.05: continue
            if math.hypot(dx - x3*IR, dy - y2*IR) <= max(0.5, (rad/90)*IR*1.1)*(0.5+z2*0.5):
                lum = max(lum, 0.95*(0.5 + 0.5*z2))
        lum *= 1 - 0.62 * smooth(0.80*IR, IR, r)
        return lum
    lum = 0.90 - 0.16 * smooth(IR, W, r)
    edge = abs(dy) / max(lid_h, 1e-6)
    lum *= 1 - 0.75 * smooth(0.78, 1.0, edge)
    lum *= 1 - 0.22 * smooth(0.82, 1.0, t)
    lum *= 1 - 0.22 * smooth(0.18, 0.0, t)
    return max(lum, 0.08)
...
dest = os.path.join(os.environ.get("OSIRIS_DIR", os.path.expanduser("~/osiris")), "public", "pythia-logo.png")
open(dest, "wb").write(png)
print(f"{dest} · {len(png)//1024} KB · {OUT}px")
